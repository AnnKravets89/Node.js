...
class UserController {
    public async getAll(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const data = await userService.getAll(validateQuery);
    ...
    public async blockUsers(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const data = await userService.blockUsers(userId);
            res.status(StatusCodesEnum.OK).json(data);
        } catch (e) {
            next(e);
        }
    ...
    public async unBlockUsers(req: Request, res: Response, next: NextFunction) {
        try {
            const { id: userId } = req.params;
            const { userId: myId } = req.res.locals
                .tokenPayload as ITokenPayload;
            ...
            const data = await userService.unBlockUsers(userId);
            res.status(StatusCodesEnum.OK).json(data);
        } catch (e) {
            next(e);
        }
